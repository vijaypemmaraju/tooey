...
[Card, [[T, 'Hello']], { bg: '#f0f0f0' }]`},errorBoundaries:{description:"catch render errors and show fallback ui",interface:`interface ErrorBoundaryNode {
  ...
  `,document.head.appendChild(e)},ie={name:"logger",onStateChange(e,t,a){["searchQuery","searchResults"].includes(e)||console.log(`[docs] ${e}:`,t,"\u2192",a)}},Z=e=>{let t=e.size||32;return[m,"",{w:t,h:t,s:{display:"inline-block"},id:`logo-${Math.random().toString(36).slice(2,8)}`}]},K=()=>{document.querySelectorAll('[id^="logo-"]').forEach(e=>{if(e.innerHTML)return;let t=parseInt(e.style.width)||32;e.innerHTML=`<svg width="${t}" height="${t}" viewBox="0 0 64 64" style="color: var(--accent);">
    ...
    </svg>`})},r=(e={},t=[])=>[i,t,{cls:"bg-bg-secondary p-4 rounded-lg border border-border"}],H=new Map,d=e=>{let t=`code-${Math.random().toString(36).slice(2,8)}`;return H.set(t,{code:e.code,lang:e.lang||"javascript"}),[m,"",{id:t,cls:"bg-code-bg rounded overflow-auto max-h-[300px]"}]},re=e=>e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),ce=()=>{H.forEach(({code:e,lang:t},a)=>{let o=document.getElementById(a);o&&!o.querySelector("pre")&&(o.innerHTML=`<pre style="margin:0;padding:8px;font-size:12px;line-height:1.4;"><code class="language-${t}">${re(e)}</code></pre>`)}),H.clear()},p=e=>[i,[[n,e.title,{cls:"text-xs font-normal text-text-muted uppercase tracking-wider block"}],e.subtitle?[n,e.subtitle,{cls:"text-sm text-text-secondary mt-1"}]:null].filter(Boolean),{cls:"mb-4"}],Q=e=>{let{item:t,type:a}=e,o=[[c,[[n,a,{cls:"text-[10px] text-accent bg-bg-tertiary px-2 py-0.5 rounded uppercase"}],[n,t.name||t.op||t.fullName||"",{cls:"text-lg font-semibold text-text-primary"}]],{cls:"gap-2 items-center flex"}],[n,t.description,{cls:"text-text-secondary my-2 mb-4 block"}]];return t.signature&&o.push(d({code:t.signature})),t.example&&o.push([i,[[n,"example",{cls:"text-xs text-text-muted uppercase tracking-wider"}],d({code:t.example})],{cls:"gap-2 my-4"}]),r({},o)},le={home:()=>[i,[[i,[[c,[[m,[Z({size:48})],{cls:"w-12 h-12 drop-shadow-[0_4px_20px_rgba(0,170,255,0.3)]"}],[i,[[n,"tooey",{cls:"text-2xl font-bold text-text-primary"}],[n,"token-efficient ui for llm output",{cls:"text-sm text-text-secondary"}]],{cls:"gap-1"}]],{cls:"gap-4 items-center flex"}],[c,[[n,"~39%",{cls:"text-accent font-semibold"}],[n,"fewer tokens",{cls:"text-text-secondary"}],[n,"|",{cls:"text-border mx-2"}],[n,"~10kb",{cls:"text-accent font-semibold"}],[n,"minified",{cls:"text-text-secondary"}],[n,"|",{cls:"text-border mx-2"}],[n,"0",{cls:"text-accent font-semibold"}],[n,"deps",{cls:"text-text-secondary"}]],{cls:"gap-1.5 items-center flex-wrap flex my-6"}]],{cls:"mb-8"}],r({},[p({title:"quick start"}),d({code:`import { render, V, T, B } from '@tooey/ui';
render(document.getElementById('app'), {
  s: { count: 0 },
  r: [V, [[T, { $: 'count' }], [B, '+', { c: 'count+' }]], { g: 8 }]
});`}),[c,[[I,"examples",{c:()=>Y("examples"),cls:"bg-accent text-white px-4 py-2 rounded border-none cursor-pointer hover:bg-accent-hover transition-colors"}],[J,"github",{href:"https://github.com/vijaypemmaraju/tooey",cls:"text-text-secondary px-4 py-2 no-underline hover:text-accent transition-colors"}]],{cls:"gap-2 flex mt-4"}]]),[j,[r({},[[n,"signals & reactivity",{cls:"font-semibold text-text-primary text-sm block mb-2"}],[n,"fine-grained reactivity with signals, computed, and effects",{cls:"text-text-secondary text-sm"}]]),r({},[[n,"function components",{cls:"font-semibold text-text-primary text-sm block mb-2"}],[n,"create reusable components as simple functions",{cls:"text-text-secondary text-sm"}]]),r({},[[n,"theming",{cls:"font-semibold text-text-primary text-sm block mb-2"}],[n,"token-based theming with $token syntax",{cls:"text-text-secondary text-sm"}]]),r({},[[n,"plugins",{cls:"font-semibold text-text-primary text-sm block mb-2"}],[n,"extend functionality with lifecycle hooks",{cls:"text-text-secondary text-sm"}]])],{cols:2,g:16,cls:"my-6"}],r({},[p({title:"components"}),[j,["layout","text","form","table","list","media"].map(e=>[i,[[n,e,{cls:"text-accent text-xs uppercase tracking-wider"}],[n,s.components.filter(t=>t.category===e).map(t=>t.name).join(" "),{cls:"text-text-primary font-mono"}]],{cls:"gap-1"}]),{cols:3,g:16}]])],{cls:"space-y-6"}],"core-functions":()=>[i,[p({title:"core functions",subtitle:"essential functions for rendering and state management"}),...s.coreFunctions.map(e=>Q({item:e,type:"function"}))],{g:16}],"instance-methods":()=>[i,[p({title:"instance methods",subtitle:"methods on TooeyInstance returned by render()"}),...s.instanceMethods.map(e=>Q({item:e,type:"method"}))],{g:16}],components:()=>[i,[p({title:"components",subtitle:"22 built-in components with short names"}),...["layout","text","form","table","list","media"].map(e=>r({},[[n,e,{cls:"text-sm font-semibold text-accent uppercase mb-3 block"}],[j,s.components.filter(t=>t.category===e).map(t=>[i,[[c,[[n,t.name,{cls:"text-success font-bold font-mono text-base"}],[n,`(${t.fullName})`,{cls:"text-text-muted text-xs"}]],{cls:"gap-1.5 items-center flex"}],[n,t.description,{cls:"text-text-secondary text-xs"}],[n,t.element,{cls:"text-text-muted text-xs font-mono"}]],{cls:"gap-1 p-2 bg-bg-tertiary rounded"}]),{cols:2,g:8}]]))],{cls:"space-y-4"}],props:()=>[i,[p({title:"props",subtitle:"all style and element properties"}),...["spacing","sizing","colors","borders","positioning","typography","layout","misc","element"].map(e=>r({},[[n,e,{cls:"text-sm font-semibold text-accent uppercase mb-3 block"}],[i,s.props.filter(t=>t.category===e).map(t=>[c,[[n,t.name,{cls:"text-success font-semibold font-mono text-sm w-10"}],[n,t.fullName||"",{cls:"text-text-primary text-sm w-[120px]"}],[n,t.description,{cls:"text-text-secondary text-xs flex-1"}],[n,t.example||"",{cls:"text-text-muted font-mono text-xs"}]],{cls:"gap-2 items-center flex py-1.5 border-b border-border"}]),{cls:"space-y-0"}]]))],{cls:"space-y-4"}],events:()=>[i,[p({title:"events & operations",subtitle:"event handlers and state operations"}),r({},[[n,"events",{cls:"text-sm font-semibold text-accent uppercase mb-3 block"}],[i,s.events.map(e=>[c,[[n,e.name,{cls:"text-success font-semibold font-mono text-sm w-10"}],[n,e.fullName||"",{cls:"text-text-primary text-sm w-[100px]"}],[n,e.description,{cls:"text-text-secondary text-xs flex-1"}],[n,e.example||"",{cls:"text-text-muted font-mono text-xs"}]],{cls:"gap-2 items-center flex py-2 border-b border-border"}]),{cls:"space-y-0"}]]),r({},[[n,"state operations",{cls:"text-sm font-semibold text-accent uppercase mb-3 block"}],[i,s.stateOps.map(e=>[c,[[n,e.op||"",{cls:"text-warning font-bold font-mono text-base w-[30px] text-center"}],[n,e.name,{cls:"text-text-primary text-sm w-20"}],[n,e.description,{cls:"text-text-secondary text-xs flex-1"}],[n,e.example||"",{cls:"text-text-muted font-mono text-xs"}]],{cls:"gap-2 items-center flex py-2 border-b border-border"}]),{cls:"space-y-0"}]])],{cls:"space-y-4"}],"control-flow":()=>[i,[p({title:"control flow",subtitle:"conditional rendering and list iteration"}),...s.controlFlow.map(e=>r({},[[n,e.name,{cls:"text-sm font-semibold text-text-primary block mb-2"}],[n,e.description,{cls:"text-text-secondary text-sm block mb-3"}],d({code:e.example||""})]))],{cls:"space-y-4"}],theming:()=>[i,[p({title:"theming",subtitle:"token-based theming system"}),r({},[[n,s.theming.description,{cls:"text-text-secondary block mb-4"}],[n,"interface",{cls:"text-xs text-text-muted uppercase tracking-wider"}],d({code:s.theming.interface})]),r({},[[n,"example",{cls:"text-xs text-text-muted uppercase tracking-wider mb-2 block"}],d({code:s.theming.example})])],{cls:"space-y-4"}],plugins:()=>[i,[p({title:"plugins",subtitle:"extend with lifecycle hooks"}),r({},[[n,s.plugins.description,{cls:"text-text-secondary block mb-4"}],d({code:s.plugins.interface})]),r({},[[n,"hooks",{cls:"text-sm font-semibold text-accent uppercase mb-3 block"}],[i,s.plugins.hooks.map(e=>[c,[[n,e.name,{cls:"text-success font-semibold font-mono text-sm w-[120px]"}],[n,e.description,{cls:"text-text-secondary text-xs flex-1"}]],{cls:"gap-2 items-center flex py-2 border-b border-border"}]),{cls:"space-y-0"}]]),r({},[[n,"example",{cls:"text-xs text-text-muted uppercase tracking-wider mb-2 block"}],d({code:s.plugins.example})])],{cls:"space-y-4"}],"function-components":()=>[i,[p({title:"function components",subtitle:"create reusable components"}),r({},[[n,s.functionComponents.description,{cls:"text-text-secondary block mb-4"}],d({code:s.functionComponents.signature})]),r({},[[n,"example",{cls:"text-xs text-text-muted uppercase tracking-wider mb-2 block"}],d({code:s.functionComponents.example})])],{cls:"space-y-4"}],"error-boundaries":()=>[i,[p({title:"error boundaries",subtitle:"catch render errors gracefully"}),r({},[[n,s.errorBoundaries.description,{cls:"text-text-secondary block mb-4"}],d({code:s.errorBoundaries.interface})]),r({},[[n,"example",{cls:"text-xs text-text-muted uppercase tracking-wider mb-2 block"}],d({code:s.errorBoundaries.example})])],{cls:"space-y-4"}],types:()=>[i,[p({title:"types",subtitle:"typescript type definitions"}),...s.types.map(e=>r({},[[n,e.name,{cls:"text-success font-semibold font-mono text-sm"}],[n,e.description,{cls:"text-text-secondary text-sm my-2"}],d({code:e.signature||""})]))],{cls:"space-y-4"}],examples:()=>[i,[p({title:"examples",subtitle:"interactive demos with token comparisons"}),[i,s.examples.map(e=>r({},[[c,[[n,e.name,{cls:"text-text-primary font-semibold text-base"}],[n,e.savings,{cls:"text-success font-bold font-mono text-sm"}]],{cls:"justify-between items-center flex"}],[n,e.description,{cls:"text-text-secondary text-sm my-2 mb-4"}],[m,[[i,[[c,[[n,"tooey",{cls:"text-accent text-xs uppercase tracking-wider"}],[n,`(${e.tooeyTokens} tokens)`,{cls:"text-text-muted text-xs"}]],{cls:"gap-2 items-center flex"}],d({code:e.tooeyCode,lang:"javascript"})],{cls:"gap-2"}],[i,[[c,[[n,"react",{cls:"text-warning text-xs uppercase tracking-wider"}],[n,`(${e.reactTokens} tokens)`,{cls:"text-text-muted text-xs"}]],{cls:"gap-2 items-center flex"}],d({code:e.reactCode,lang:"jsx"})],{cls:"gap-2"}]],{cls:"grid grid-cols-1 sm:grid-cols-2 gap-4"}],[n,"live demos",{cls:"text-text-muted text-xs uppercase tracking-wider mt-4 mb-2"}],[m,[[i,[[n,"tooey",{cls:"text-accent text-[10px] uppercase tracking-wider"}],[m,"",{id:`demo-tooey-${e.id}`,cls:"bg-bg-tertiary p-4 rounded-lg border border-border min-h-[100px]"}]],{cls:"gap-2"}],[i,[[n,"react",{cls:"text-warning text-[10px] uppercase tracking-wider"}],[m,"",{id:`demo-react-${e.id}`,cls:"bg-bg-tertiary p-4 rounded-lg border border-border min-h-[100px]"}]],{cls:"gap-2"}]],{cls:"grid grid-cols-1 sm:grid-cols-2 gap-4"}]])),{cls:"space-y-6"}]],{cls:"space-y-4"}]},M={},D={};s.examples.forEach(e=>{M[e.id]=e.demoSpec,D[e.id]=e.reactDemoCode});var h=[{label:"home",page:"home"},{label:"core functions",page:"core-functions"},{label:"instance methods",page:"instance-methods"},{label:"components",page:"components"},{label:"props",page:"props"},{label:"events & ops",page:"events"},{label:"control flow",page:"control-flow"},{label:"theming",page:"theming"},{label:"plugins",page:"plugins"},{label:"function components",page:"function-components"},{label:"error boundaries",page:"error-boundaries"},{label:"types",page:"types"},{label:"examples",page:"examples"}],q=E(window.location.hash.slice(1)||"home"),V=E(""),v=E([]),f=E(window.matchMedia("(prefers-color-scheme: dark)").matches),L,b,Y=e=>{window.location.hash=e,q.set(e),V.set(""),v.set([])},X=!1,S=null,ee=e=>{S||(S=document.createElement("style"),document.head.appendChild(S)),S.textContent=e?`
  ...
  `},de=()=>X?Promise.resolve():new Promise(e=>{ee(f());let t=document.createElement("script");t.src="https://unpkg.com/prismjs@1/prism.min.js",t.onload=()=>{let a=document.createElement("script");a.src="https://unpkg.com/prismjs@1/components/prism-jsx.min.js",a.onload=()=>{let o=document.createElement("script");o.src="https://unpkg.com/prismjs@1/components/prism-typescript.min.js",o.onload=()=>{X=!0,e()},document.head.appendChild(o)},document.head.appendChild(a)},document.head.appendChild(t)}),pe=()=>{let e=window.Prism;e&&document.querySelectorAll('code[class*="language-"]').forEach(t=>{t.classList.contains("prism-highlighted")||(e.highlightElement(t),t.classList.add("prism-highlighted"))})},U=!1,me=()=>U?Promise.resolve():new Promise(e=>{let t=document.createElement("script");t.src="https://unpkg.com/react@18/umd/react.development.js",t.crossOrigin="anonymous",t.onload=()=>{let a=document.createElement("script");a.src="https://unpkg.com/react-dom@18/umd/react-dom.development.js",a.crossOrigin="anonymous",a.onload=()=>{let o=document.createElement("script");o.src="https://unpkg.com/@babel/standalone/babel.min.js",o.onload=()=>{U=!0,e()},document.head.appendChild(o)},document.head.appendChild(a)},document.head.appendChild(t)}),ge=(e,t)=>{try{let a=window.Babel,o=window.React,k=window.ReactDOM;if(!a||!o||!k)return;let g=a.transform(t,{presets:["react"]}).code,w=new Function("React",`${g}; return ${t.match(/function\s+(\w+)/)?.[1]||"Component"};`)(o);k.createRoot(e).render(o.createElement(w)),e.dataset.rendered="true"}catch(a){console.warn("[tooey] failed to render react demo:",a),e.textContent="failed to render"}},ue=()=>{if(!L)return;let e=q();L.innerHTML="",B(L,{r:le[e]()}),K(),requestAnimationFrame(()=>{ce(),de().then(()=>{requestAnimationFrame(pe)})}),document.querySelectorAll(".nav-btn").forEach((t,a)=>{let o=t;o.style.background=h[a].page===e?"var(--bg-tertiary)":"transparent",o.style.color=h[a].page===e?"var(--accent)":"var(--text-secondary)"}),e==="examples"&&(Object.keys(M).forEach(t=>{let a=document.getElementById(`demo-tooey-${t}`);if(a&&!a.dataset.rendered)try{let o=JSON.parse(M[t]);B(a,o),a.dataset.rendered="true"}catch(o){console.warn("[tooey] failed to render demo:",t,o)}}),me().then(()=>{Object.keys(D).forEach(t=>{let a=document.getElementById(`demo-react-${t}`);a&&!a.dataset.rendered&&ge(a,D[t])})}))},N=()=>{if(!b)return;let e=v();if(!e.length){b.innerHTML="";return}b.innerHTML="",B(b,{r:[i,e.slice(0,10).map(t=>[m,[[c,[[n,t.type,{cls:"text-[9px] text-accent bg-bg-tertiary px-1.5 py-0.5 rounded uppercase"}],[n,t.name,{cls:"text-text-primary font-medium text-sm"}]],{cls:"gap-1.5 items-center flex"}],[n,t.description,{cls:"text-text-secondary text-xs"}]],{cls:"search-result p-2 rounded cursor-pointer hover:bg-bg-tertiary transition-colors"}]),{cls:"absolute top-[42px] left-0 right-0 bg-bg-secondary rounded p-2 z-[100] max-h-[300px] overflow-auto border border-border shadow-lg"}]})},fe=()=>{_(f()?$:G),se();let e=document.getElementById("app");if(!e)return;let t={s:{},r:[c,[[I,"",{id:"menu-btn",cls:"fixed top-4 left-4 z-[1001] bg-bg-secondary p-2.5 rounded-lg border border-border hidden"}],[m,"",{id:"sidebar-overlay",cls:"fixed inset-0 z-[999] bg-black/50 hidden"}],[i,[[c,[Z({size:32}),[n,"tooey",{cls:"text-lg font-bold text-text-primary"}]],{cls:"gap-2 items-center flex mb-6"}],[i,[[ae,"",{ph:"search...",id:"search",cls:"bg-bg-tertiary text-text-primary px-3 py-2 rounded w-full border border-border outline-none focus:border-accent"}]],{cls:"relative mb-4"}],[m,"",{id:"search-results"}],[i,h.map(l=>[I,l.label,{cls:"nav-btn bg-transparent text-text-secondary px-3 py-2 rounded w-full text-left text-sm cursor-pointer border-none hover:bg-bg-tertiary hover:text-accent transition-colors"}]),{cls:"gap-0.5"}],[c,[[I,"",{id:"theme-btn",cls:"bg-transparent text-text-secondary p-2 rounded cursor-pointer border-none hover:text-accent transition-colors"}],[J,"",{href:"https://github.com/vijaypemmaraju/tooey",id:"github-link",cls:"text-text-secondary p-2 hover:text-accent transition-colors"}]],{cls:"gap-2 mt-auto"}]],{id:"sidebar",cls:"w-60 h-screen p-6 bg-bg-secondary fixed top-0 left-0 z-[1000] border-r border-border overflow-y-auto flex flex-col transition-transform duration-300"}],[m,[{boundary:!0,child:[m,"",{id:"page-content"}],fallback:r({},[[n,"error",{cls:"text-error font-semibold"}]]),onError:l=>console.error(l)}],{id:"main-content",cls:"ml-60 p-8 max-w-[900px] min-h-screen"}]],{cls:"w-full"}]};B(e,t,{plugins:[ie]}),L=document.getElementById("page-content"),b=document.getElementById("search-results");let a=document.getElementById("search"),o=document.getElementById("theme-btn"),k=document.getElementById("github-link"),g=document.getElementById("menu-btn"),w=document.getElementById("sidebar"),T=document.getElementById("sidebar-overlay"),x=document.getElementById("main-content"),P='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/></svg>',z='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/></svg>',te='<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.3 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61-.546-1.386-1.332-1.755-1.332-1.755-1.089-.744.083-.729.083-.729 1.205.085 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.605-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 21.795 24 17.295 24 12c0-6.63-5.37-12-12-12z"/></svg>',R='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 12h18M3 6h18M3 18h18"/></svg>',ne='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18M6 6l12 12"/></svg>';o.innerHTML=f()?P:z,k.innerHTML=te,g.innerHTML=R;let A=window.innerWidth<=768,u=!1,C=()=>{A=window.innerWidth<=768,A?(g.style.display="block",w.style.transform=u?"translateX(0)":"translateX(-100%)",T.style.display=u?"block":"none",x.style.marginLeft="0",x.style.padding="16px",x.style.paddingTop="60px"):(g.style.display="none",w.style.transform="translateX(0)",T.style.display="none",x.style.marginLeft="240px",x.style.padding="32px",u=!1)},oe=()=>{u=!u,g.innerHTML=u?ne:R,C()},O=()=>{u&&(u=!1,g.innerHTML=R,C())};g.addEventListener("click",oe),T.addEventListener("click",O),window.addEventListener("resize",C),C(),a?.addEventListener("input",l=>{let y=l.target.value;V.set(y),v.set(F(y)),N()}),a?.addEventListener("keydown",l=>{l.key==="Escape"&&(V.set(""),v.set([]),a.value="",N())}),document.addEventListener("click",l=>{!l.target.closest("#search")&&!l.target.closest("#search-results")&&(v.set([]),N())}),document.querySelectorAll(".nav-btn").forEach((l,y)=>{l.addEventListener("click",()=>{Y(h[y].page),O()})}),o.addEventListener("click",()=>{f.set(!f()),_(f()?$:G),o.innerHTML=f()?P:z,ee(f())}),window.addEventListener("popstate",()=>{let l=window.location.hash.slice(1)||"home";h.some(y=>y.page===l)&&q.set(l)}),W(ue),W(N),K()};document.addEventListener("DOMContentLoaded",fe);
