...
}`,file:"09-wizard.html"}]};function q(e){if(!e||e.length<2)return[];let n=e.toLowerCase(),o=[];return i.coreFunctions.forEach(a=>{(a.name.toLowerCase().includes(n)||a.description.toLowerCase().includes(n))&&o.push({type:"function",...a})}),i.instanceMethods.forEach(a=>{(a.name.toLowerCase().includes(n)||a.description.toLowerCase().includes(n))&&o.push({type:"method",...a})}),i.components.forEach(a=>{(a.name.toLowerCase().includes(n)||a.fullName?.toLowerCase().includes(n)||a.description.toLowerCase().includes(n))&&o.push({type:"component",...a})}),i.props.forEach(a=>{(a.name.toLowerCase().includes(n)||a.fullName?.toLowerCase().includes(n)||a.description.toLowerCase().includes(n))&&o.push({type:"prop",...a})}),i.events.forEach(a=>{(a.name.toLowerCase().includes(n)||a.fullName?.toLowerCase().includes(n)||a.description.toLowerCase().includes(n))&&o.push({type:"event",...a})}),i.stateOps.forEach(a=>{(a.op?.includes(n)||a.name.toLowerCase().includes(n)||a.description.toLowerCase().includes(n))&&o.push({type:"operation",...a})}),i.controlFlow.forEach(a=>{(a.name.toLowerCase().includes(n)||a.description.toLowerCase().includes(n))&&o.push({type:"controlflow",...a})}),i.types.forEach(a=>{(a.name.toLowerCase().includes(n)||a.description.toLowerCase().includes(n))&&o.push({type:"type",...a})}),o}var L={colors:{bg:"#0a0a0f",bgSecondary:"#111118",bgTertiary:"#1a1a24",bgHover:"#252530",text:"#e0e0e0",textSecondary:"#888",textMuted:"#666",accent:"#0af",success:"#4f8",warning:"#fa0",error:"#f55",border:"#333",codeBg:"#111"},spacing:{xs:4,sm:8,md:16,lg:24,xl:32},radius:{sm:4,md:8,lg:12}},A={colors:{bg:"#fff",bgSecondary:"#f8f9fa",bgTertiary:"#e9ecef",bgHover:"#dee2e6",text:"#212529",textSecondary:"#495057",textMuted:"#868e96",accent:"#0066cc",success:"#28a745",warning:"#ffc107",error:"#dc3545",border:"#dee2e6",codeBg:"#f4f4f4"},spacing:L.spacing,radius:L.radius},P=e=>{let n=document.documentElement;Object.entries(e.colors).forEach(([o,a])=>{n.style.setProperty(`--${o.replace(/([A-Z])/g,"-$1").toLowerCase()}`,a)})},K={name:"logger",onStateChange(e,n,o){["searchQuery","searchResults"].includes(e)||console.log(`[docs] ${e}:`,n,"\u2192",o)}},G=e=>{let n=e.size||32;return[Z,[{tag:"rect",x:24,y:8,width:16,height:48,rx:8,fill:"currentColor"},{tag:"rect",x:12,y:20,width:40,height:14,rx:7,fill:"currentColor",transform:"rotate(-20 32 27)"}],{w:n,h:n,vb:"0 0 64 64",fg:"var(--accent)"}]},r=(e={},n=[])=>[s,n,{bg:"var(--bg-secondary)",p:16,r:8,s:{border:"1px solid var(--border)"}}],p=e=>[m,[[t,e.code,{s:{whiteSpace:"pre-wrap",wordBreak:"break-word"}}]],{bg:"var(--code-bg)",p:8,r:4,fs:12,ff:"ui-monospace, monospace",fg:"var(--success)",ov:"auto",s:{maxHeight:"300px"}}],l=e=>[s,[[t,e.title,{fs:11,fw:400,fg:"var(--text-muted)",s:{textTransform:"uppercase",letterSpacing:"1px",display:"block"}}],e.subtitle?[t,e.subtitle,{fs:13,fg:"var(--text-secondary)",m:"4px 0 0 0"}]:null].filter(Boolean),{m:"0 0 16px 0"}],O=e=>{let{item:n,type:o}=e,a=[[c,[[t,o,{fs:10,fg:"var(--accent)",bg:"var(--bg-tertiary)",p:"2px 8px",r:4,s:{textTransform:"uppercase"}}],[t,n.name||n.op||n.fullName||"",{fs:18,fw:600,fg:"var(--text)"}]],{g:8,ai:"c"}],[t,n.description,{fg:"var(--text-secondary)",m:"8px 0 16px 0",s:{display:"block"}}]];return n.signature&&a.push(p({code:n.signature})),n.example&&a.push([s,[[t,"example",{fs:11,fg:"var(--text-muted)",s:{textTransform:"uppercase",letterSpacing:"1px"}}],p({code:n.example})],{g:8,m:"16px 0"}]),r({},a)},ee={home:()=>[s,[[s,[[c,[[m,[G({size:48})],{w:48,h:48,s:{filter:"drop-shadow(0 4px 20px rgba(0,170,255,0.3))"}}],[s,[[t,"tooey",{fs:28,fw:700,fg:"var(--text)"}],[t,"token-efficient ui for llm output",{fs:14,fg:"var(--text-secondary)"}]],{g:4}]],{g:16,ai:"c"}],[c,[[t,"~39%",{fg:"var(--accent)",fw:600}],[t,"fewer tokens",{fg:"var(--text-secondary)"}],[t,"|",{fg:"var(--border)",m:"0 8px"}],[t,"~10kb",{fg:"var(--accent)",fw:600}],[t,"minified",{fg:"var(--text-secondary)"}],[t,"|",{fg:"var(--border)",m:"0 8px"}],[t,"0",{fg:"var(--accent)",fw:600}],[t,"deps",{fg:"var(--text-secondary)"}]],{g:6,ai:"c",flw:"wrap",m:"24px 0"}]],{m:"0 0 32px 0"}],r({},[l({title:"quick start"}),p({code:`import { render, V, T, B } from '@tooey/ui';
render(document.getElementById('app'), {
  s: { count: 0 },
  r: [V, [[T, { $: 'count' }], [B, '+', { c: 'count+' }]], { g: 8 }]
});`}),[c,[[N,"examples",{c:()=>X("examples"),bg:"var(--accent)",fg:"#fff",p:"8px 16px",r:4,s:{border:"none",cursor:"pointer"}}],[W,"github",{href:"https://github.com/vijaypemmaraju/tooey",fg:"var(--text-secondary)",p:"8px 16px",s:{textDecoration:"none"}}]],{g:8,m:"16px 0 0 0"}]]),[b,[r({},[[t,"signals & reactivity",{fw:600,fg:"var(--text)",fs:14,s:{display:"block",marginBottom:"8px"}}],[t,"fine-grained reactivity with signals, computed, and effects",{fg:"var(--text-secondary)",fs:13}]]),r({},[[t,"function components",{fw:600,fg:"var(--text)",fs:14,s:{display:"block",marginBottom:"8px"}}],[t,"create reusable components as simple functions",{fg:"var(--text-secondary)",fs:13}]]),r({},[[t,"theming",{fw:600,fg:"var(--text)",fs:14,s:{display:"block",marginBottom:"8px"}}],[t,"token-based theming with $token syntax",{fg:"var(--text-secondary)",fs:13}]]),r({},[[t,"plugins",{fw:600,fg:"var(--text)",fs:14,s:{display:"block",marginBottom:"8px"}}],[t,"extend functionality with lifecycle hooks",{fg:"var(--text-secondary)",fs:13}]])],{cols:2,g:16,m:"24px 0"}],r({},[l({title:"components"}),[b,["layout","text","form","table","list","media"].map(e=>[s,[[t,e,{fg:"var(--accent)",fs:11,s:{textTransform:"uppercase",letterSpacing:"1px"}}],[t,i.components.filter(n=>n.category===e).map(n=>n.name).join(" "),{fg:"var(--text)",ff:"monospace"}]],{g:4}]),{cols:3,g:16}]])],{g:24}],"core-functions":()=>[s,[l({title:"core functions",subtitle:"essential functions for rendering and state management"}),...i.coreFunctions.map(e=>O({item:e,type:"function"}))],{g:16}],"instance-methods":()=>[s,[l({title:"instance methods",subtitle:"methods on TooeyInstance returned by render()"}),...i.instanceMethods.map(e=>O({item:e,type:"method"}))],{g:16}],components:()=>[s,[l({title:"components",subtitle:"22 built-in components with short names"}),...["layout","text","form","table","list","media"].map(e=>r({},[[t,e,{fs:13,fw:600,fg:"var(--accent)",s:{textTransform:"uppercase",marginBottom:"12px",display:"block"}}],[b,i.components.filter(n=>n.category===e).map(n=>[s,[[c,[[t,n.name,{fg:"var(--success)",fw:700,ff:"monospace",fs:16}],[t,`(${n.fullName})`,{fg:"var(--text-muted)",fs:12}]],{g:6,ai:"c"}],[t,n.description,{fg:"var(--text-secondary)",fs:12}],[t,n.element,{fg:"var(--text-muted)",fs:11,ff:"monospace"}]],{g:4,p:8,bg:"var(--bg-tertiary)",r:4}]),{cols:2,g:8}]]))],{g:16}],props:()=>[s,[l({title:"props",subtitle:"all style and element properties"}),...["spacing","sizing","colors","borders","positioning","typography","layout","misc","element"].map(e=>r({},[[t,e,{fs:13,fw:600,fg:"var(--accent)",s:{textTransform:"uppercase",marginBottom:"12px",display:"block"}}],[s,i.props.filter(n=>n.category===e).map(n=>[c,[[t,n.name,{fg:"var(--success)",fw:600,ff:"monospace",fs:13,w:40}],[t,n.fullName||"",{fg:"var(--text)",fs:13,w:120}],[t,n.description,{fg:"var(--text-secondary)",fs:12,s:{flex:"1"}}],[t,n.example||"",{fg:"var(--text-muted)",ff:"monospace",fs:11}]],{g:8,ai:"c",p:"6px 0",s:{borderBottom:"1px solid var(--border)"}}]),{g:0}]]))],{g:16}],events:()=>[s,[l({title:"events & operations",subtitle:"event handlers and state operations"}),r({},[[t,"events",{fs:13,fw:600,fg:"var(--accent)",s:{textTransform:"uppercase",marginBottom:"12px",display:"block"}}],[s,i.events.map(e=>[c,[[t,e.name,{fg:"var(--success)",fw:600,ff:"monospace",fs:14,w:40}],[t,e.fullName||"",{fg:"var(--text)",fs:13,w:100}],[t,e.description,{fg:"var(--text-secondary)",fs:12,s:{flex:"1"}}],[t,e.example||"",{fg:"var(--text-muted)",ff:"monospace",fs:11}]],{g:8,ai:"c",p:"8px 0",s:{borderBottom:"1px solid var(--border)"}}]),{g:0}]]),r({},[[t,"state operations",{fs:13,fw:600,fg:"var(--accent)",s:{textTransform:"uppercase",marginBottom:"12px",display:"block"}}],[s,i.stateOps.map(e=>[c,[[t,e.op||"",{fg:"var(--warning)",fw:700,ff:"monospace",fs:16,w:30,ta:"center"}],[t,e.name,{fg:"var(--text)",fs:13,w:80}],[t,e.description,{fg:"var(--text-secondary)",fs:12,s:{flex:"1"}}],[t,e.example||"",{fg:"var(--text-muted)",ff:"monospace",fs:11}]],{g:8,ai:"c",p:"8px 0",s:{borderBottom:"1px solid var(--border)"}}]),{g:0}]])],{g:16}],"control-flow":()=>[s,[l({title:"control flow",subtitle:"conditional rendering and list iteration"}),...i.controlFlow.map(e=>r({},[[t,e.name,{fs:14,fw:600,fg:"var(--text)",s:{display:"block",marginBottom:"8px"}}],[t,e.description,{fg:"var(--text-secondary)",fs:13,s:{display:"block",marginBottom:"12px"}}],p({code:e.example||""})]))],{g:16}],theming:()=>[s,[l({title:"theming",subtitle:"token-based theming system"}),r({},[[t,i.theming.description,{fg:"var(--text-secondary)",s:{display:"block",marginBottom:"16px"}}],[t,"interface",{fs:11,fg:"var(--text-muted)",s:{textTransform:"uppercase",letterSpacing:"1px"}}],p({code:i.theming.interface})]),r({},[[t,"example",{fs:11,fg:"var(--text-muted)",s:{textTransform:"uppercase",letterSpacing:"1px",marginBottom:"8px",display:"block"}}],p({code:i.theming.example})])],{g:16}],plugins:()=>[s,[l({title:"plugins",subtitle:"extend with lifecycle hooks"}),r({},[[t,i.plugins.description,{fg:"var(--text-secondary)",s:{display:"block",marginBottom:"16px"}}],p({code:i.plugins.interface})]),r({},[[t,"hooks",{fs:13,fw:600,fg:"var(--accent)",s:{textTransform:"uppercase",marginBottom:"12px",display:"block"}}],[s,i.plugins.hooks.map(e=>[c,[[t,e.name,{fg:"var(--success)",fw:600,ff:"monospace",fs:13,w:120}],[t,e.description,{fg:"var(--text-secondary)",fs:12,s:{flex:"1"}}]],{g:8,ai:"c",p:"8px 0",s:{borderBottom:"1px solid var(--border)"}}]),{g:0}]]),r({},[[t,"example",{fs:11,fg:"var(--text-muted)",s:{textTransform:"uppercase",letterSpacing:"1px",marginBottom:"8px",display:"block"}}],p({code:i.plugins.example})])],{g:16}],"function-components":()=>[s,[l({title:"function components",subtitle:"create reusable components"}),r({},[[t,i.functionComponents.description,{fg:"var(--text-secondary)",s:{display:"block",marginBottom:"16px"}}],p({code:i.functionComponents.signature})]),r({},[[t,"example",{fs:11,fg:"var(--text-muted)",s:{textTransform:"uppercase",letterSpacing:"1px",marginBottom:"8px",display:"block"}}],p({code:i.functionComponents.example})])],{g:16}],"error-boundaries":()=>[s,[l({title:"error boundaries",subtitle:"catch render errors gracefully"}),r({},[[t,i.errorBoundaries.description,{fg:"var(--text-secondary)",s:{display:"block",marginBottom:"16px"}}],p({code:i.errorBoundaries.interface})]),r({},[[t,"example",{fs:11,fg:"var(--text-muted)",s:{textTransform:"uppercase",letterSpacing:"1px",marginBottom:"8px",display:"block"}}],p({code:i.errorBoundaries.example})])],{g:16}],types:()=>[s,[l({title:"types",subtitle:"typescript type definitions"}),...i.types.map(e=>r({},[[t,e.name,{fg:"var(--success)",fw:600,ff:"monospace",fs:14}],[t,e.description,{fg:"var(--text-secondary)",fs:13,m:"8px 0"}],p({code:e.signature||""})]))],{g:16}],examples:()=>[s,[l({title:"examples",subtitle:"interactive demos with token comparisons"}),[s,i.examples.map(e=>r({},[[c,[[t,e.name,{fg:"var(--text)",fw:600,fs:16}],[t,e.savings,{fg:"var(--success)",fw:700,ff:"monospace",fs:14}]],{jc:"sb",ai:"c"}],[t,e.description,{fg:"var(--text-secondary)",fs:13,m:"8px 0 16px 0"}],[b,[[s,[[c,[[t,"tooey",{fg:"var(--accent)",fs:11,s:{textTransform:"uppercase",letterSpacing:"1px"}}],[t,`(${e.tooeyTokens} tokens)`,{fg:"var(--text-muted)",fs:11}]],{g:8,ai:"c"}],p({code:e.tooeyCode})],{g:8}],[s,[[c,[[t,"react",{fg:"var(--warning)",fs:11,s:{textTransform:"uppercase",letterSpacing:"1px"}}],[t,`(${e.reactTokens} tokens)`,{fg:"var(--text-muted)",fs:11}]],{g:8,ai:"c"}],[m,[[t,e.reactCode,{s:{whiteSpace:"pre-wrap",wordBreak:"break-word"}}]],{bg:"var(--code-bg)",p:8,r:4,fs:12,ff:"ui-monospace, monospace",fg:"var(--warning)",ov:"auto",s:{maxHeight:"300px"}}]],{g:8}]],{cols:2,g:16}],[t,"live demos",{fg:"var(--text-muted)",fs:11,s:{textTransform:"uppercase",letterSpacing:"1px"},m:"16px 0 8px 0"}],[b,[[s,[[t,"tooey",{fg:"var(--accent)",fs:10,s:{textTransform:"uppercase",letterSpacing:"1px"}}],[m,"",{id:`demo-tooey-${e.id}`,bg:"var(--bg-tertiary)",p:16,r:8,s:{border:"1px solid var(--border)",minHeight:"100px"},"data-spec":e.demoSpec}]],{g:8}],[s,[[t,"react",{fg:"var(--warning)",fs:10,s:{textTransform:"uppercase",letterSpacing:"1px"}}],[m,"",{id:`demo-react-${e.id}`,bg:"var(--bg-tertiary)",p:16,r:8,s:{border:"1px solid var(--border)",minHeight:"100px"},"data-react":e.reactDemoCode}]],{g:8}]],{cols:2,g:16}]])),{g:24}]],{g:16}]},$=[{label:"home",page:"home"},{label:"core functions",page:"core-functions"},{label:"instance methods",page:"instance-methods"},{label:"components",page:"components"},{label:"props",page:"props"},{label:"events & ops",page:"events"},{label:"control flow",page:"control-flow"},{label:"theming",page:"theming"},{label:"plugins",page:"plugins"},{label:"function components",page:"function-components"},{label:"error boundaries",page:"error-boundaries"},{label:"types",page:"types"},{label:"examples",page:"examples"}],Q=R("home"),E=R(""),v=R([]),y=R(window.matchMedia("(prefers-color-scheme: dark)").matches),B,h,X=e=>{Q.set(e),E.set(""),v.set([])},F=!1,te=()=>F?Promise.resolve():new Promise(e=>{let n=document.createElement("script");n.src="https://unpkg.com/react@18/umd/react.development.js",n.crossOrigin="anonymous",n.onload=()=>{let o=document.createElement("script");o.src="https://unpkg.com/react-dom@18/umd/react-dom.development.js",o.crossOrigin="anonymous",o.onload=()=>{let a=document.createElement("script");a.src="https://unpkg.com/@babel/standalone/babel.min.js",a.onload=()=>{F=!0,e()},document.head.appendChild(a)},document.head.appendChild(o)},document.head.appendChild(n)}),ne=(e,n)=>{try{let o=window.Babel,a=window.React,g=window.ReactDOM;if(!o||!a||!g)return;let f=o.transform(n,{presets:["react"]}).code,w=new Function("React",`${f}; return ${n.match(/function\s+(\w+)/)?.[1]||"Component"};`)(a);g.createRoot(e).render(a.createElement(w)),e.dataset.rendered="true"}catch(o){console.warn("[tooey] failed to render react demo:",o),e.textContent="failed to render"}},ae=()=>{if(!B)return;let e=Q();B.innerHTML="",I(B,{r:ee[e]()}),document.querySelectorAll(".nav-btn").forEach((n,o)=>{let a=n;a.style.background=$[o].page===e?"var(--bg-tertiary)":"transparent",a.style.color=$[o].page===e?"var(--accent)":"var(--text-secondary)"}),e==="examples"&&(document.querySelectorAll("[data-spec]").forEach(n=>{let o=n,a=o.getAttribute("data-spec");if(a&&!o.dataset.rendered)try{let g=JSON.parse(a);I(o,g),o.dataset.rendered="true"}catch(g){console.warn("[tooey] failed to render demo:",g)}}),te().then(()=>{document.querySelectorAll("[data-react]").forEach(n=>{let o=n,a=o.getAttribute("data-react");a&&!o.dataset.rendered&&ne(o,a)})}))},C=()=>{if(!h)return;let e=v();if(!e.length){h.innerHTML="";return}h.innerHTML="",I(h,{r:[s,e.slice(0,10).map(n=>[m,[[c,[[t,n.type,{fs:9,fg:"var(--accent)",bg:"var(--bg-tertiary)",p:"2px 6px",r:2,s:{textTransform:"uppercase"}}],[t,n.name,{fg:"var(--text)",fw:500,fs:13}]],{g:6,ai:"c"}],[t,n.description,{fg:"var(--text-secondary)",fs:11}]],{p:8,r:4,cur:"pointer",cls:"search-result"}]),{pos:"abs",t:42,l:0,rt:0,bg:"var(--bg-secondary)",r:4,p:8,z:100,mh:300,ov:"auto",s:{border:"1px solid var(--border)",boxShadow:"0 4px 20px rgba(0,0,0,0.3)"}}]})},oe=()=>{P(y()?L:A);let e=document.getElementById("app");if(!e)return;let n={s:{},r:[c,[[N,"",{id:"menu-btn",pos:"fix",t:16,l:16,z:1001,bg:"var(--bg-secondary)",p:10,r:8,s:{border:"1px solid var(--border)",display:"none"}}],[m,"",{id:"sidebar-overlay",pos:"fix",t:0,l:0,w:"100vw",h:"100vh",z:999,bg:"rgba(0,0,0,0.5)",s:{display:"none"}}],[s,[[c,[G({size:32}),[t,"tooey",{fs:18,fw:700,fg:"var(--text)"}]],{g:8,ai:"c",m:"0 0 24px 0"}],[s,[[Y,"",{ph:"search...",bg:"var(--bg-tertiary)",fg:"var(--text)",p:"8px 12px",r:4,w:"100%",s:{border:"1px solid var(--border)",outline:"none"},id:"search"}]],{pos:"rel",m:"0 0 16px 0"}],[m,"",{id:"search-results"}],[s,$.map(d=>[N,d.label,{bg:"transparent",fg:"var(--text-secondary)",p:"8px 12px",r:4,w:"100%",ta:"left",fs:13,cur:"pointer",s:{border:"none"},cls:"nav-btn"}]),{g:2}],[c,[[N,"",{bg:"transparent",fg:"var(--text-secondary)",p:8,r:4,cur:"pointer",s:{border:"none"},id:"theme-btn"}],[W,"",{href:"https://github.com/vijaypemmaraju/tooey",fg:"var(--text-secondary)",p:8,id:"github-link"}]],{g:8,m:"auto 0 0 0"}]],{w:240,h:"100vh",p:24,bg:"var(--bg-secondary)",pos:"fix",t:0,l:0,z:1e3,s:{borderRight:"1px solid var(--border)",overflowY:"auto",display:"flex",flexDirection:"column",transition:"transform 0.3s ease"},id:"sidebar"}],[m,[{boundary:!0,child:[m,"",{id:"page-content"}],fallback:r({},[[t,"error",{fg:"var(--error)",fw:600}]]),onError:d=>console.error(d)}],{m:"0 0 0 240px",p:32,mw:900,s:{minHeight:"100vh"},id:"main-content"}]],{w:"100%"}]};I(e,n,{plugins:[K]}),B=document.getElementById("page-content"),h=document.getElementById("search-results");let o=document.getElementById("search"),a=document.getElementById("theme-btn"),g=document.getElementById("github-link"),f=document.getElementById("menu-btn"),w=document.getElementById("sidebar"),T=document.getElementById("sidebar-overlay"),x=document.getElementById("main-content"),D='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/></svg>',M='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/></svg>',_='<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.3 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61-.546-1.386-1.332-1.755-1.332-1.755-1.089-.744.083-.729.083-.729 1.205.085 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.605-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 21.795 24 17.295 24 12c0-6.63-5.37-12-12-12z"/></svg>',H='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 12h18M3 6h18M3 18h18"/></svg>',U='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18M6 6l12 12"/></svg>';a.innerHTML=y()?D:M,g.innerHTML=_,f.innerHTML=H;let j=window.innerWidth<=768,u=!1,k=()=>{j=window.innerWidth<=768,j?(f.style.display="block",w.style.transform=u?"translateX(0)":"translateX(-100%)",T.style.display=u?"block":"none",x.style.marginLeft="0",x.style.padding="16px",x.style.paddingTop="60px"):(f.style.display="none",w.style.transform="translateX(0)",T.style.display="none",x.style.marginLeft="240px",x.style.padding="32px",u=!1)},J=()=>{u=!u,f.innerHTML=u?U:H,k()},V=()=>{u&&(u=!1,f.innerHTML=H,k())};f.addEventListener("click",J),T.addEventListener("click",V),window.addEventListener("resize",k),k(),o?.addEventListener("input",d=>{let S=d.target.value;E.set(S),v.set(q(S)),C()}),o?.addEventListener("keydown",d=>{d.key==="Escape"&&(E.set(""),v.set([]),o.value="",C())}),document.addEventListener("click",d=>{!d.target.closest("#search")&&!d.target.closest("#search-results")&&(v.set([]),C())}),document.querySelectorAll(".nav-btn").forEach((d,S)=>{d.addEventListener("click",()=>{X($[S].page),V()})}),a.addEventListener("click",()=>{y.set(!y()),P(y()?L:A),a.innerHTML=y()?D:M}),z(ae),z(C)};document.addEventListener("DOMContentLoaded",oe);
